...
A subscribed journey also needs this session's own identity, so that a second agent window open on the same machine can never answer for this one. Codex exposes it as `CODEX_THREAD_ID` — the same marker its hooks report as `session_id` — and the skills pass it explicitly on every journey read. If no real session marker can be read, a bound lesson says so and waits instead of guessing; the free standalone method and every editor tool keep working either way.
