...
- `learning/scratch/` — the small exercises you wrote to meet each new concept before it went into the project
...
- **Do the typing yourself.** When the tutor dictates a command, you run it in your terminal. When a concept is new to you, you meet it outside the project first: a short explanation with a tiny example, then a couple of small exercises you write and run in `learning/scratch/`, predicting the output before each run, then a question: another one, or into the project? In the project, you write every line of what it's teaching; the tutor only writes the structure it isn't teaching (imports, a function's signature, config), tells you what each piece is for, and marks the places for your code with `TODO(you)` lines. Replace each marker line with your code in your editor and hit save — it's watching the file, not the chat, and it looks the moment the markers are replaced with your code. Stuck? Say "hint" and you get one rung at a time — where to look, then the shape of the answer, then the code — never the answer unasked. The one exception: a chunk built entirely from concepts you've already mastered may be written for you — then you review the diff before it's committed, like a teammate's PR. And a task built entirely from concepts you've marked known opens with an offer to do exactly that; say yes, or say you'd rather type it — it never just does it.
- **Tune how you're taught.** On a subscribed journey, three settings travel with it: how many drills per new concept (0 to 3), the step size (how much the tutor spells out before you write: a line, a function, or a whole feature), and prediction checks (only on runs that use new material, or every run). Change them in the journey's settings on the web, or just tell the tutor in plain words ("stop the drills", "just tell me what each line should do"). If you like exact commands, the same three are `altitude teaching set drills=1`, `altitude teaching set step_size=line`, and `altitude teaching set check_density=every_run`. The tutor also asks a short check-in at the close of a lesson: after your second task, then about every fifth, and never more than once in a sitting. It names one thing it noticed, proposes one change, and asks yes or no; a yes takes effect from your next lesson, a no leaves things as they are. Switching between Beginner and Intermediate stays on the web.
...
- You write every line of what you're learning. New concepts get drilled in a scratch file before they touch your project; the tutor writes only the structure it isn't teaching, and says so.
- Predict before you run anything that uses a concept you're learning. A wrong prediction is the best teacher you'll meet, and "not sure" is an honest answer.
- Review questions come only from what you've already learned, never from the lesson you're about to have.
