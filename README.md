...
- **Tune how you're taught.** On a subscribed journey, three settings travel with it: how many drills per new concept (`drills`, 0 to 3), how much the tutor specifies before you write (`step_size`: a line, a function, or a whole feature), and how often it asks you to predict (`check_density`: only when new material runs, or every run). Change them in the journey's settings on the web, or just tell the tutor in plain words ("stop the drills", "just tell me what each line should do"). Every few lessons the tutor asks a short check-in at the close: one thing it noticed, one proposed change, yes or no. Your answer is what it records. Switching between Beginner and Intermediate stays on the web.
