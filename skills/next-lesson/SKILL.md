...
- `"cache"` → choose a mode below exactly as with a live answer. If `reason` is present, add at most one calm line that this lesson is working from the last-synced copy — no warning, no troubleshooting.
