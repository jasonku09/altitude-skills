...
- **Paused subscription:** the binding resolves to this project but `entitled` is false. Use free mode for this session, against the existing plan; the free-mode reference file carries the paused-session specifics.
...
Paid mode may owe one or two closing lines about updates after that recap; the paid-mode reference file governs when and how, and free mode says nothing about updates.
