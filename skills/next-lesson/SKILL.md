...
In free mode, give the same immediate accommodation and record the concept as `understood` in `learning/knowledge-graph.md` with the evidence note `self-reported prior knowledge`. There is no account mutation or map handoff in standalone free mode.
