...
- **Version evidence** — the read reached Altitude (`source` is `"network"`) and still carries no `learning_runtime`; `learning_runtime.status` is `update_required`; the CLI rejected `--session`, `--task`, or `--plan-revision` as an unknown flag; or `source` is `"none"` with no `reason` at all. Take the update-required path: say plainly that this workshop's Altitude tools are behind what the lesson needs, ask them to run `altitude update` in their own terminal and to update the Altitude plugin in their agent, then run `/altitude:next-lesson` again. Do not re-run a rejected command with older flags, and do not read an absent `learning_runtime` as permission to teach the lesson the legacy way.
- **Reach failure** — `source` is `"none"` with any `reason`, or a `"cache"` answer that carries no runtime. The read never got an answer to carry a runtime in, so it says nothing about their version: never tell them their tools are out of date here. Take Step 1's exit for that `reason`, hold the bound lesson until a read supports it, and change nothing on disk.
