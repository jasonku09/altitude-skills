...
- `"cache"` → continue to the account routes below exactly as with a live answer. If `reason` is present, add at most one calm line that you are working from the last-synced copy — no warning, no troubleshooting.
